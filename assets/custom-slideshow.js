...
class ImageSlideshow extends HTMLElement {
    constructor() {
      ...
      const { altArrows, autoplay, autoHeight, allowTouchMove, draggable, zoom, grabCursor, slidesPerView, thumbnails,  a11y, freeMode, pagination, navigation, loop, disableOn, spaceBetween, centeredSlides, breakpoints, numberPagination, effect, controlContainer, fadeOnLargeUp, hoverArrowNav} = this.attributes;
      ...
      waitForSwiper().then(() => {
              if (this.options.thumbnails) {
                setTimeout(() => {
                  ...
                  const controlElement = document.querySelector("[" + this.options.thumbnails + "]");
                  this.options.thumbs = {
                    swiper: controlElement.getSwiper(),
                  };
                  this.swiper = new Swiper(this.querySelector('.swiper-container'), this.options);
                  if (typeof enableZoomOnHover !== 'undefined' && this.options.zoom == true) {
                    enableZoomOnHover(2);
                  }
          ...
          const event = new CustomEvent('swiperLoaded', { detail: { swiper: this.swiper } });
